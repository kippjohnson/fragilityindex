...
First we perform an initial basic logistic regression. (For `survivalfragility` this must be a formula which can be passed to `coxph()` and for `linearfragility` this must be a formula which can be passed to `lm()`. These formulas should not have interaction terms.)
